...
import (
	...
	"github.com/HwGin-original/gin-admin/server/src/bll"
	"github.com/HwGin-original/gin-admin/server/src/ginplus"
	"github.com/HwGin-original/gin-admin/server/src/schema"
...
// NewUser 创建用户管理控制器
func NewUser(b *bll.Common) *User {
	return &User{
		UserBll: b.User,
	}
}
...
func (a *User) QueryPage(c *gin.Context) {
	...
	if v := util.S(c.Query("status")).Int(); v > 0 {
		params.Status = v
	}
	...
	items, pr, err := a.UserBll.QueryPage(ginplus.NewContext(c), params, ginplus.GetPaginationParam(c))
	if err != nil {
